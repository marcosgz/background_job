...
RSpec.describe BackgroundJob::UniqueJob do
  ...
  describe '.to_hash' do
    ...
    context 'with lock data' do
      let(:lock) { BackgroundJob::Lock.new(digest: 'digest', lock_id: 'jid', ttl: 10) }
  ...
  describe '.coerce' do
    ...
    context 'with lock data' do
      specify do
        ...
        expect(model.lock).to eq(BackgroundJob::Lock.new(digest:'digest', lock_id:'jid', ttl:10))
      end
  ...
  describe '.lock=' do
    ...
    specify do
      lock = BackgroundJob::Lock.new(digest: 'digest', lock_id: 'jid', ttl: 10)
      model.lock = lock
