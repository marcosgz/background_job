...
RSpec.describe 'BackgroundJob::Middleware::UniqueJob::Faktory', freeze_at: [2020, 7, 2, 12, 30, 50] do
  let(:job_class) do
    Class.new do
      def self.name
        'DummyFaktoryWorker'
  ...
  let(:job) do
    BackgroundJob::Jobs::Faktory.new(job_class.name,
      queue: 'mailer',
  ...
  shared_examples 'unique job across' do |across|
    ...
    context "with a unique job across #{across}" do
      let(:lock_digest) do
        BackgroundJob::LockDigest.new(:faktory, :mailer, across: across).to_s
      end
      ...
      it 'does not push job when active uniqueness lock exist' do
        redis do |conn|
          ...
          # Adds a lock_id
          lock_id = Digest::SHA256.hexdigest('["DummyFaktoryWorker",[]]')
          ...
          with_uniq_payload = job.payload.merge(
            'uniq' => {
              'across' => across.to_s,
              'timeout' => 3_600,
          ...
          expect { |y| BackgroundJob::Middleware::UniqueJob::Faktory::Worker.new.call(job_class.new, with_uniq_payload, &y) }.to yield_control
          expect(conn.zcount(lock_digest, 0, now + DAY_IN_SECONDS)).to eq(0)
        end
      end
      ...
      it 'adds a lock with 1 hour of TTL and enqueues a new job' do
        redis do |conn|
          ...
          Timecop.travel(travel_to) do
            new_result = job.push
            expect(Faktory::Queues['mailer'].size).to eq(2)
            expect(conn.zcount(lock_digest, 0, now + DAY_IN_SECONDS)).to eq(1)
            ttl = conn.zscore(lock_digest, lock_id)
            expect(ttl.to_i).to eq(travel_to.to_i + 3_600)

            # Removes the lock after job execution
            expect { |y| BackgroundJob::Middleware::UniqueJob::Faktory::Worker.new.call(job_class.new, result, &y) }.to yield_control
            expect(conn.zcount(lock_digest, 0, now + DAY_IN_SECONDS)).to eq(0)
          end
        end
      end
  ...
  describe '.call' do
    ...
    after do
      ...
      BackgroundJob.config.faktory.middleware.clear
      reset_config!
    end
